...
public class CombatAgent extends Agent {
	 ...
	 * @param enemies
	 *            The player numbers of enemies
	 * @param wanderWhileIdle
	 *            Whether to move in random directions when you have nothing
	 *            better to do.
	 * @param verbose
	 *            Verbosity
	 */
	...
	/**
	 * Set the parameters to the default values. For enemies, they cannot be
	 * immediately set, so leave them as null to be interpreted later.
	 */
	...
	/**
	 * Start a new trial. Uses the StateView, which contains information in
	 * logs, resources, and units Some of the unit information may be in the
	 * template
	 * 
	 * @param newstate
	 * @return
	...
	public Map<Integer, Action> initialStep(StateView newstate,
		...
		if (enemies == null) {
			// actually count the enemies, just in case there is some kind of
			// duplicate or if your player number isn't there
			int numenemies = 0;
	...
	public Map<Integer, Action> middleStep(StateView newstate,
		...
		// Read in the logs for every step that occurred since it was last this
		// player's turn
		for (int stepToRead = lastStepMovedIn; stepToRead < newstate
				.getTurnNumber(); stepToRead++) {
			...
			for (DeathLog death : statehistory.getDeathLogs(stepToRead)) {
				...
				// check if anyone is attacking the dead unit, and tell them to
				// stop
				for (Map.Entry<Integer, Action> order : unitOrders.entrySet()) {

					if (order.getValue() != null) {
						Action attackthedeadunit = Action.createCompoundAttack(
								order.getKey(), death.getDeadUnitID());
			...
			if (verbose) {
				...
				for (DamageLog damagereport : statehistory
						.getDamageLogs(stepToRead)) {
					if (damagereport.getAttackerController() == playernum) {
						writeLineVisual(damagereport.getAttackerID() + " hit "
								+ damagereport.getDefenderID() + " for "
								+ damagereport.getDamage() + " damage");
					}
					if (damagereport.getDefenderController() == playernum) {
						writeLineVisual(damagereport.getDefenderID()
								+ " was hit by " + damagereport.getAttackerID()
								+ " for " + damagereport.getDamage()
								+ " damage");
					}

				}
			}
			// Update it's list of orders by checking for completions and
			// failures and removing those
			for (ActionResult feedback : statehistory.getCommandFeedback(
					playernum, stepToRead).values()) {

				if (feedback.getFeedback() != ActionFeedback.INCOMPLETE)// Everything
																		// but
																		// incomplete
																		// is
																		// some
																		...
																		// complete
				{
					// because the feedback mixes primitive feedback on
					// duratives and compound feedback on primitives, need to
					// check if it is the right action
					Action action = feedback.getAction();
					int unitid = action.getUnitId();
					Action order = unitOrders.get(unitid); // if this gives
															// nullpointer, then
															// there was some
															// failure in
															// registering units
	...
	private Map<Integer, Action> getAction(StateView currentstate) {
		...
		for (Map.Entry<Integer, Action> order : unitOrders.entrySet()) {
			if (verbose)
				writeLineVisual("Combat Agent for plr " + playernum
						+ "'s order: " + order.getKey() + " is to use "
						+ order.getValue());
	...
	private void doAggro(StateView state) {
		for (Map.Entry<Integer, Action> order : unitOrders.entrySet()) {
			...
			{
				// check all of the other units to check for an enemy that is in
				// sight range
				...
				for (int enemy : enemies) {
					...
					for (Integer enemyUnitID : state.getUnitIds(enemy)) {
						...
						// get the chebyshev distance (which is the base
						// distance for warcraft 2)
						if (sightradius > DistanceMetrics.chebyshevDistance(ux,
								uy, enemyUnit.getXPosition(),
								enemyUnit.getYPosition())) {
							// (if you can see it)
							foundsomething = true;
							unitOrders.put(order.getKey(), Action
									.createCompoundAttack(order.getKey(),
											enemyUnitID));
							break;
						}
					}
				...
				if (!foundsomething) {
					...
					if (wanderwhenidle) {
						Direction direction = Direction.values()[(int) (Math
								.random() * Direction.values().length)];
						int newx = ux + direction.xComponent();
						int newy = uy + direction.yComponent();
						Action a = Action.createCompoundMove(u.getID(), newx,
								newy);
